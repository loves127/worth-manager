<template>
  <div>
    <v-search @openDialog="openDialog"
              ref="vSearch"
              @onSearch="onSearch"/>
    ...
    <ftooter-pages :pagination="pagination"
  ...
  export default {
    ...
    methods: {
      ...
      openDialog(_title, _id) {
        ...
        if (this.panelTitle === '新增公司') {
          this.form = {};
        } else {
          const res = CompanyApi.findById({_id: _id})
          res.then(data => {
            if (data.state === '200') {
              this.form = data.data
            }
      ...
      getList() {
        ...
        this.vloading = true;
        const res = CompanyApi.list(params);
        res.then((data) => {
          if (data.state === '200') {
      ...
      onSubmit() {
        if (this.panelTitle === '新增公司') {
          ...
          let res = CompanyApi.update(param);
          res.then(() => {
            this.$message({
              message: '更新成功',
              type: 'success'
      ...
      deleteMany() {
        ...
        this.$confirm('此操作将删除选中公司, 是否继续?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          let res = CompanyApi.delete({'_ids': this.multipleSelection});
          res.then((data) => {
            if (data.state === '200') {
              that.$message({
                type: 'success',
