    ...
    <transition name="form-fade" mode="in-out">
      <div class="login-form" v-show="showLogin">
        <div class="login-logo">
          <img src="https://ss3.bdstatic.com/70cFv8Sh_Q1YnxGkpoWK1HF6hhy/it/u=745315900,1003984187&fm=27&gp=0.jpg"> <span>未来社区管理平台</span>
        </div>
        <el-form :model="formLogin" :rules="rulesLogin" ref="formLogin">
          <el-form-item prop="username">
            <el-input type="text" v-model="formLogin.username" auto-complete="off" @keyup.enter.native="submitForm('formLogin')">
            </el-input>
          </el-form-item>
          <el-form-item prop="password">
            <el-input  type="password" v-model="formLogin.password" auto-complete="off" @keyup.enter.native="submitForm('formLogin')" >
              <i slot="suffix" class="iconfont icon-yonghu1"></i>
            </el-input>
          </el-form-item>
          <el-form-item>
        ...
        <div class="login-account">
          <!--<span>账号：admin</span><span>密码：admin</span>-->
        </div>
      </div>
    </transition>
  ...
  export default {
    ...
    methods: {
      ...
      async goLogin () { // 登录
        ...
        const res = await ManagerApi.goLogin(params);
        if (res.state === '200') {
          let $sto = that.$sto;
          let $conf = that.$conf;
          // 模拟登录成功返回的Token
