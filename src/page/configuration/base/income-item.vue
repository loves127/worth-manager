<template>
  <div class="income-item">
    <header>
      ...
      <el-button type="primary" @click="openDialog('新增收入类型')">新增</el-button>
    </header>
    <el-table
      :data="tableData"
      v-loading="vloading"
      ...
      <el-table-column label="操作" width="200">
        <template slot-scope="scope">
          <el-button
            size="small"
            @click="openDialog('编辑收入类型',scope.row._id)">
    ...
    <el-dialog
      ...
      <el-form :model="form" :label-position="labelPosition">
        ...
        <el-form-item label="类型" :label-width="formLabelWidth">
          <el-input v-model="form.incomeType" auto-complete="off" placeholder="请输入收入类型"/>
        </el-form-item>
      </el-form>
  ...
  export default {
    ...
    methods: {
      ...
      openDialog(_title, _id) {
        ...
        if (this.panelTitle === '新增收入类型') {
          this.form = {};
        } else {
          const res = IncomeTypeApi.findById({_id: _id})
          res.then(data => {
            if (data.state === '200') {
              this.form = data.data
            }
      ...
      onSubmit() {
        if (this.panelTitle === '新增收入类型') {
          ...
          let res = IncomeTypeApi.add(params);
          res.then((data) => {
            if (data.state === '200') {
              this.$message({
                message: '新增成功',
          ...
          let res = IncomeTypeApi.update(param);
          res.then(() => {
            this.$message({
              message: '更新成功',
              type: 'success'
      ...
      getList(params) {
        this.vloading = true;
        const res = IncomeTypeApi.list(params);
        res.then((data) => {
          if (data.state === '200') {
      ...
      handleDelete(row) {
        const id = row._id;
        let that = this;
        this.$confirm('此操作将删除该收入类型, 是否继续?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          const res = IncomeTypeApi.delete({_ids: id});
          res.then((data) => {
            if (data.state === '200') {
              that.$message({
                type: 'success',
